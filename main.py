...
def amount_handler(message: telebot.types.Message, curr_from, curr_to):
    ...
    try:
        ...
        answer_text = f'Стоимость {amount} единиц валюты {curr_from} в валюте {curr_to}: {conv}\n{amount} \
...
/help. Или воспользуйся командой /convert.')
        ...
        currency_from = currency_from.capitalize()
        currency_to = currency_to.capitalize()
        try:
            conv = Converter.get_convert(currency_from, currency_to, amount)
            answer_text = f'Стоимость {amount} единиц валюты {currency_from} в валюте {currency_to}: {conv}\n{amount} \
{currencies[currency_from]} = {conv} {currencies[currency_to]}'
